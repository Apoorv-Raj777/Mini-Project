        ...
        for cell, bands in aggs.items():
            for band_name, v in bands.items():
                ...
                if numeric_conf >= 0.8 and v["N"] >= 8:
                    ...
                    conf_cat = "low"
                out.append({
                    "cell_id": v["cell_id"], "band": band_name,
                    "lat": v["lat"], "lng": v["lng"],
    ...
    try:
        ...
        for cell,bands in aggs.items():
            for band_id,v in bands.items():
                out.append({
                    "cell_id": v["cell_id"], "band": v["band"],
                    "lat": v["lat"], "lng": v["lng"],
                    "score": v["score"], "confidence": v["confidence"],
                    "sample_count": v["N"], "last_updated": v["last_ts"]
                })
        return jsonify(out)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        for cell,bands in aggs.items():
            for b, info in bands.items():
                cell_lat = info["lat"]; cell_lng = info["lng"]
                if cell_lat is None or cell_lng is None: continue
                if not (min_lat <= cell_lat <= max_lat and min_lng <= cell_lng <= max_lng): continue
                d = haversine_m(lat, lng, cell_lat, cell_lng)
                if d <= radius_m:
                    results.append({
                        "cell_id": info["cell_id"], "band": b, "lat": cell_lat, "lng": cell_lng,
                        "score": info["score"], "confidence": info["confidence"], "sample_count": info["N"],
